...
function _showUploadToast(message, type = '') {
  uploadToast.className = `upload-toast${type ? ' ' + type : ''}`;
  uploadToastText.textContent = message;
  uploadToast.hidden = false;
  if (_toastTimer) clearTimeout(_toastTimer);
  if (type === 'success' || type === 'error') {
    _toastTimer = setTimeout(() => { uploadToast.hidden = true; }, 3500);
  }
}
...
async function _handleFileUpload(file) {
  ...
  if (file.size > MAX_MB * 1024 * 1024) {
    _showUploadToast(`File too large. Max ${MAX_MB} MB.`, 'error');
    return;
  }
  ...
  _showUploadToast(`Ingesting "${file.name}"…`);
  ...
  try {
    ...
    if (!res.ok) {
      _showUploadToast(data.detail || 'Upload failed.', 'error');
      return;
    }

    // Show success toast
    _showUploadToast(
      `✓ "${data.filename}" ingested — ${data.chunks} chunks, ~${data.words} words`,
      'success'
    );

    // Inject a friendly confirmation bubble into chat
    addMsg('assistant',
    ...
    _showUploadToast('Network error during upload.', 'error');
  } finally {
...
fileInput.addEventListener('change', () => {
  _handleFileUpload(fileInput.files[0]);
});

// Drag & drop onto the entire chat area
const chatWrap = document.querySelector('.chat-wrap');
if (chatWrap) {
  chatWrap.addEventListener('dragover', (e) => {
    e.preventDefault();
    chatWrap.style.outline = '2px dashed rgba(167,139,250,0.5)';
  });
  chatWrap.addEventListener('dragleave', () => {
    chatWrap.style.outline = '';
  });
